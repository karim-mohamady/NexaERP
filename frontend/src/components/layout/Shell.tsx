      ...
      <div className="lg:ps-72">
        <header className="sticky top-0 z-20 flex h-16 items-center gap-3 border-b border-slate-200 bg-white/90 px-4 backdrop-blur dark:border-slate-800 dark:bg-slate-950/90">
          ...
          <div className="relative">
            <button onClick={() => setNotificationOpen((value) => !value)} className="relative grid size-10 place-items-center rounded-md border border-slate-200 dark:border-slate-800" aria-label="Notifications"><Bell size={18} />{unread > 0 && <span className="absolute -right-1 -top-1 grid size-5 place-items-center rounded-full bg-rose-600 text-[10px] font-bold text-white">{unread}</span>}</button>
            {notificationOpen && <div className="absolute end-0 top-12 z-50 w-80 rounded-lg border border-slate-200 bg-white p-3 shadow-xl dark:border-slate-800 dark:bg-slate-950">
              <div className="flex items-center justify-between"><strong>{t(lang, 'notifications')}</strong><button onClick={async () => { await api.post('/notifications/mark-all-read'); await loadNotifications() }} className="text-xs text-cyan-600">Mark all read</button></div>
              <div className="mt-3 max-h-80 space-y-2 overflow-auto">{notifications.length === 0 ? <p className="p-3 text-sm text-slate-500">No notifications.</p> : notifications.map((item) => <button key={item.id} onClick={() => navigate('/app/notifications')} className="block w-full rounded-md bg-slate-50 p-3 text-left text-sm hover:bg-slate-100 dark:bg-slate-900 dark:hover:bg-slate-800"><span className="font-medium">{item.title}</span><span className="mt-1 block text-xs text-slate-500">{item.body}</span></button>)}</div>
            </div>}
          </div>
